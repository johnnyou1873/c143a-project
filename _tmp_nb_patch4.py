...
insert_marker = "BOOKS_DIR = ROOT / 'notebooks' / 'data' / 'LibriSpeech' / 'books' / 'ascii'"
if any(insert_marker in ''.join(c.get('source','')) for c in nb['cells']):
    print('Rebuild cell already present')
else:
    md = {
        'cell_type': 'markdown',
        'metadata': {},
        'source': [
            ...
            'Use LibriSpeech text in notebooks/data to regenerate data/librispeech_sentences.pkl before loading.\n'
        ],
    }
    code = {
        'cell_type': 'code',
    ...
    print('Inserted rebuild cells at', insert_at)
